...
struct InventoryFormView: View {
    ...
    var body: some View {
        ...
        .fileImporter(isPresented: .init(get: { vm.selectedUSDZSource == .fileImporter }, set: { _ in vm.selectedUSDZSource = nil }), allowedContentTypes: [UTType.usdz], onCompletion: { result in switch result {
        ...
        Section("AR Model") {
            ...
            if let progress = vm.uploadProgress,
               case let .uploading(type) =
                vm.loadingState, progress.totalUnitCount > 0 {
                VStack {
    ...
    func viewAR(url: URL) {
        ...
        let vc = UIApplication.shared.firstJeyWindow?
            .rootViewController?.presentedViewController ??
            UIApplication.shared.firstJeyWindow?
            .rootViewController
